...
async function main() {
    try {
        ...
        const userExists = await wallet.get('hodUser');
        if (userExists) {
            console.log("hodUser already exists");
            return;
        }
        ...
        const secret = await ca.register({
            ...
            enrollmentID: 'hodUser',
            role: 'client',
            attrs: [
                { name: 'role', value: 'HOD', ecert: true }
            ]
        }, adminUser);
        ...
        await wallet.put('hodUser', x509Identity);
        console.log("Successfully registered and enrolled HOD User");
